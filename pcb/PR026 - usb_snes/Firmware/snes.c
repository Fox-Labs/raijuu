...
static char snesBuildReport(unsigned char *reportBuffer, char id)
{
	...
	if (reportBuffer != NULL)
	{
		...
		y = x = 128;
		if (lrcb1&0x1) { x = 255; } //Clock 8 = Right on joypad
		if (lrcb1&0x2) { x = 0; } //Clock 7 = Left on joypad
		
		if (lrcb1&0x4) { y = 255; } //Clock 6 = Down on joypad
		if (lrcb1&0x8) { y = 0; } //Clock 5 = Up on joypad
