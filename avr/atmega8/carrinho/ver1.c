...
uint16_t sensor(){
    ...
    adc4 = readADC(1) - A4;
    adc5 = readADC(0) - A5;
    e = ( ( (adc1 * -2.0) + (adc2 * -1.0) + (adc3 * 0.0) + (adc4 * 1.0) + (adc5 * 2.0) ) / (adc1 + adc2 + adc3 + adc4 + adc5) ) * 30;
    return e;
}

void motor(int out){
    if (out < 0) {ESQ = FULL + out; DIR = FULL; printf("E:%d D:%d    ", FULL + out, FULL); }
    if (out > 0) {ESQ = FULL; DIR = FULL - out; printf("E:%d D:%d    ", FULL, FULL - out); }
    if (out == 0) {ESQ = FULL; DIR = FULL; printf("E:%d D:%d    ", FULL, FULL); }
}    
